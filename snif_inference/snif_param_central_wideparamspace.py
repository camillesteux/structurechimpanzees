import sys
import os
os.chdir('/ifs/igc/folders/PCG/csteux/snif/snif-main')
sys.path.append('/ifs/igc/folders/PCG/csteux/snif/snif-main')
...
for c in clist:
    for w in [0.5, 1]:
            h_inference_parameters = InferenceParameters(
                data_source = '~/gape-psmc/FINAL_CLEAN_Pan_troglodytes_troglodytes-A957_Vaillant.psmc',
                ...
                bounds_effective_size = (1000, 20000)
            )

            h_settings = Settings(
                static_library_location = '~/snif/snif-main/libs/libsnif.so',
                custom_filename_tag = '',
                output_directory = '~/snif/Chimps/troglodytes/troglo_vaillant',
