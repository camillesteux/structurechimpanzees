...
4.3.6   100523  for --max_seed, removed nargs option as it was generating a list with possible bug
...
def main():
    ...
    ARGS_PRINT["max_seed"] = int(str(ARGS_PRINT["max_seed"]))
...
def get_args():
    ...
    parser.add_argument('--max_seed', type=np.int64, required=False, default=np.int64(1e6), help="Maximum allowed seed, should be high enough, on the order of 1e10") # 4.3.1
    ...
    # versions
    Versions = "msprime: "+str(msp.__version__)+" | "+"demes: "+str(demes.__version__)
    if ARGS["draw"] is not None:
        import demesdraw
