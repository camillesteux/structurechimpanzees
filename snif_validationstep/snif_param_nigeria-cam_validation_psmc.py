...
for c in [7]:
    for w in [0.5]:
        h_inference_parameters = InferenceParameters(
            data_source = ''.join(['~/snif/Chimps/val_psmc/input_data/231213-172353_c07YN_dA_w050_SQ_Koto_psmc_10x100Mb_-', args.individual, '.psmc']),
            source_type = SourceType.PSMC,
            IICR_type = IICRType.Exact,
            ms_reference_size = 1162,
            ...
            bounds_event_times = [(1e4/gentime, 1e5/gentime), (1.5e5/gentime, 3e5/gentime), (3.5e5/gentime, 5e5/gentime), (8e5/gentime, 1.5e6/gentime), (2e6/gentime, 6e7/gentime), (2e6/gentime, 7e6/gentime)],
            bounds_effective_size = (10, 10000)
            )

        h_settings = Settings(
            static_library_location = '~/snif/snif-main-new/libs/libsnif.so',
            custom_filename_tag = ''.join(['ellioti_valpsmc_c', str(c), 'eb_w', str(w), '_', args.individual]),
        ...
        config = Configuration(
            ...
            CG_source = '~/snif/Chimps/val_psmc/input_data/231213-172353_c07YN_dA_w050_SQ_Koto_inferred-model-005.json',
